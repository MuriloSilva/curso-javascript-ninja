...
(() => {
    ...
    */
    let numberObjects = Array(10).fill(null).map((item, index) => ({ number: index + 1 }))
    ...
    */
    const operation = justMod2Or3.reduce((acum, item) => (acum + 1) * item, 0)
    console.log('\nOperation:', operation);
    // ?
    ...
    console.
    */
    const operation2 = justMod2Or3.reduceRight((acum, item) => (acum + 1) * item, 0)
    ...
    */
    ...
    console.log('\nSeu nome na língua do "P":', name.reduce((acum, item) => `${acum}P${item}`, ''));
    // ?
    ...
    */
    const inversedName = name.reduceRight((a, b) => a + b)
