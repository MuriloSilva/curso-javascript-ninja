...
(() => {
    ...
    */
    ...
    const cleanCPF = (cpf) => cpf.replace(/^(\d{3})[\s-.x]+(\d{3})[\s-.x]+(\d{3,4})[\s-.x]+(\d{1,2})$/g, `$1$2$3$4`);    
    [
        '049 214 3421 1',
        '010.458.522-05',
        '735 500 794 - 22',
        '101.123-131x32'
    ].forEach(item => console.log(cleanCPF(item)))
    ...
    */
    ...
    [
        ...
        .map(item => item.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, `$1.$2.$3-$4`))
    ...
    */
    ...
    const months = /ju[nl]ho/g
    const frase = 'Os meses de janeiro, junho e julho começam com a letra j.'
    console.log(frase.match(months))
    ...
    */
    ...
    const html = '<h1>Título da página</h1><p>Este é um parágrafo</p><footer>Rodapé</footer>'
    console.log(html.replace(/<(\w+)>([^<]+)<\/(\w+)>/g, '<$1>O texto dentro da tag "$1" é "$2"</$3>\n'))
})()